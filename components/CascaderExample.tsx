...
export default class CascaderScreen extends Component<any, any> {
    ...
    render() {
        const optionsX = [
            ...
            { label: '中关村', id: 'zhongguancun', pId: 'haidianqu' },
            { label: '上海', id: 'shanghai' },
            { label: '黄浦区', id: 'huangpuqu', pId: 'shanghai' },
            { label: '闸北区', id: 'zhabeiqu', pId: 'shanghai' },
        ];
                    ...
                    <Cascader
                        ...
                        onChange={(value) => {
                            this.setState({ valueA: value }, () => {
                                const lastItem = this.getTargetItem(optionsA, value[value.length - 1]);
                                let isLeaf = false;
                                if (lastItem?.label.includes('饮品')) {
                                    isLeaf = true;
                                } else {
                                    isLeaf = !lastItem?.children || lastItem.children.length === 0;
                                }
                                if (isLeaf) {
                                    const message = value.join(' → ');
                                    Alert.alert(
                        ...
                        renderItem={(item) => {
                            const active = item.active;
                            const isLeaf = typeof item.isLeaf !== 'undefined'
                                ? item.isLeaf
